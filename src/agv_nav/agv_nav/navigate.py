...
class AGVNavigator(Node):
    def __init__(self):
        ...
        self.waiting_for_coordinates = False  # 좌표 입력 대기 상태 변수
        self.waypoints = [
            self.create_pose(0.072321, -0.03734, (0.0, 0.0, -0.0081218, 0.99997)),  # 첫 번째 목적지
            self.create_pose(3.2413, -1.1711, (0.0, 0.0, 0.95645, 0.29188)),  # 두 번째 목적지
    ...
    def get_result_callback(self, future):
        ...
        self.get_logger().info('다음 목적지로 이동하려면 목적지 번호 (1, 2, 3)를 입력하거나, AGV 기준으로 이동할 좌표를 입력하려면 4를 입력하세요:')
    ...
    def check_user_input(self):
        while not self.input_queue.empty():
            ...
            if self.waiting_for_coordinates:
                self.update_current_pose()  # 현재 위치 업데이트
                try:
                    x, y = map(float, user_input.split(','))
                    self.move_to_coordinates(x, y)
                    self.waiting_for_coordinates = False
                except ValueError:
                    self.get_logger().info('유효하지 않은 입력입니다. 좌표를 x,y 형식으로 입력하세요.')
            else:
                if user_input.isdigit():
                    ...
                    if 0 <= index < len(self.waypoints):
                        ...
                        self.get_logger().info('AGV의 축을 기준으로 이동할 x, y 좌표를 입력하세요 (예: 1.0,2.0):')
                        self.waiting_for_coordinates = True  # 좌표 입력 대기 상태 설정
                    else:
    ...
    def move_to_coordinates(self, x, y):
        if self.current_pose is None:
            self.get_logger().info('현재 위치를 알 수 없습니다. 이동할 수 없습니다.')
            return
        ...
        # yaw 각도와 입력받은 x, y 좌표를 사용하여 새로운 목표 위치 계산
        map_x = self.current_pose.pose.position.x + x * math.cos(yaw) - y * math.sin(yaw)
        map_y = self.current_pose.pose.position.y + x * math.sin(yaw) + y * math.cos(yaw)

        new_pose = self.create_pose(map_x, map_y, (
